...
def _parser() -> argparse.ArgumentParser:
    ...
    for name in ("disable", "enable", "delete", "test", "rotate-secret"):
...
async def _run(args: argparse.Namespace) -> int:
    ...
    if args.action == "list":
        for item in accounts.registry().list_safe():
            print(f"{item['id']}: {item['username']} at {item['host']}:{item['port']} enabled={item['enabled']}")
        return 0
    ...
    config = AccountConfig(
        args.account_id, args.host, args.port, args.username, args.auth_method
    )
